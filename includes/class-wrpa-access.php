...
/**
 * Class: WRPA_Access
 * Description: Premium access handler with one-time trial limit, start date tracking,
 * auto expiry redirect, cache purge, instant access confirmation, and LiteSpeed cache bypass.
 */

class WRPA_Access {

    public static function init() {
        add_filter('woocommerce_add_to_cart_validation', [__CLASS__, 'validate_trial_add_to_cart'], 10, 3);

        add_action('woocommerce_order_status_completed', [__CLASS__, 'grant_access']);
        add_action('woocommerce_order_status_processing', [__CLASS__, 'grant_access']);
        add_action('woocommerce_payment_complete', [__CLASS__, 'grant_access']);
        add_action('woocommerce_thankyou', [__CLASS__, 'grant_access'], 1);

        add_action('template_redirect', [__CLASS__, 'restrict_premium_pages'], 20);
    }

    /**
     * Trial ürününü yalnızca bir kere alabilme kontrolü
     */
    public static function validate_trial_add_to_cart($passed, $product_id, $quantity) {
        ...
        if ( $product_id != $trial_id ) {
            return $passed;
        }

        $user_id   = get_current_user_id();
        $has_trial = get_user_meta($user_id, 'wrpa_trial_used', true);

        if ( $has_trial ) {
            wc_add_notice(__('You have already used your free trial. Please choose a paid plan instead.'), 'error');
